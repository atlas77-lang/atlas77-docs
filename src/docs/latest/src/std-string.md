...
fun main() {
    ...
    println(upper.to_str());  // "HELLO, WORLD!"
}
```
...
fun main() {
    let csv = new String("apple,banana,cherry");
    let fruits = csv.split(",");
    
    let i = 0u;
    while i < len(&fruits) {
        println(fruits[i].to_str());
        i = i + 1u;
    }
}
```
...
fun main() {
    let str = new String("Hello");
    str.push(' ');
    str.push_str(new String("World"));
    str.push('!');
    println(str.to_str());  // "Hello World!"
}
```
...
- [std/iter](./std-iter.md) - String iteration
